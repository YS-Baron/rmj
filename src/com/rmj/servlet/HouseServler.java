...
@WebServlet(value = {"/hou/add","/hou/del","/hou/findAll","/hou/update","/hou/delImg","/hou/addImg","/hou/updateis","/hou/getid","/hou/findadmin","/hou/findUser","hou/findhid"})
public class HouseServler extends HttpServlet {
    ...
    private void doAdd(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        int res=houseService.save(houses);
        if (res > 0) {
            out.print(JsonUtil.getJsonStr(1, "增加数据成功"));
        } else {
            out.print(JsonUtil.getJsonStr(0, "增加数据失败"));
        }
    }
